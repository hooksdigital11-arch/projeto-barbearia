...
export function TemplateDeleteConfirm({ isOpen, onClose, template, onSuccess }: TemplateDeleteConfirmProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <div className="relative bg-bg-surface border border-white/5 rounded-2xl w-full max-w-[380px] p-6 text-center space-y-6">
        ...
        <div className="flex gap-3 pt-2">
          <button
            onClick={onClose}
            disabled={isPending}
            className="flex-1 py-3 rounded-full border border-white/10 text-text-primary text-sm font-bold hover:bg-white/5 transition-colors disabled:opacity-50"
          >
            Cancelar
          </button>
          <button
            ...
            disabled={isPending}
            className="flex-1 py-3 rounded-full bg-red-600 text-text-primary text-sm font-bold hover:bg-red-700 transition-colors disabled:opacity-50"
          >
